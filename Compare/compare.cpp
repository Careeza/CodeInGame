...
void  compare_algo(State start) {
    ...
    while (!s.final_state()) {
        s.do_action(algo5::best_moove(s));
        s.do_action(algo4::best_moove(s));
    }
    ...
    while (!s2.final_state()) {
        s2.do_action(algo4::best_moove(s2));
        s2.do_action(algo5::best_moove(s2));
    }
...
int     main(int argc, char **argv) {
    ...
    for (int j = 0; j < 1; j++) {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            threads[i] = std::thread(compare_algo, s);
        }
        for (int i = 0; i < 20; i++) {
            threads[i].join();
        }
    ...
    std::cout << "Fin de simulation :" << algo1_score << ", " << 40 - algo1_score << std::endl;
}