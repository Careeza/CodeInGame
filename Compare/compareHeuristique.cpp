...
void  compare_algo(State start) {
    ...
    while (!s.final_state()) {
        s.do_action(algo5::bestActionH(s));
        s.do_action(algo4::bestActionH(s));
    }
    ...
    while (!s2.final_state()) {
        s2.do_action(algo4::bestActionH(s2));
        s2.do_action(algo5::bestActionH(s2));
    }
    ...
    std::cout << "SCORE FINAL " << s2.info.score[0] << " vs " << s2.info.score[1] << std::endl;
}
